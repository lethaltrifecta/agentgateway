...
impl<'a> OidcJwtResolver<'a> {
	pub async fn resolve_oauth2_provider(
		...
		let metadata = self.client.get_metadata(ctx, issuer).await?;
		let jwk_set = self.client.fetch_jwk_set(ctx, &metadata).await?;
		let jwks_inline = serde_json::to_string(&jwk_set)
			.map_err(|e| Error::Internal(format!("failed to serialize jwks: {e}")))?;
		Ok(ResolvedOAuth2Provider {
