...
#[tokio::test]
async fn route_scoped_oauth2_wins_over_listener_fallback_and_handles_callback() {
	let Some((gateway, oidc, client)) = setup_gateway_with_listener_and_route_oauth2(true).await
	else {
		return;
	};
	...
	let (auth_url, state, nonce_cookie) = start_auth(&client, &gateway, "/private/data").await;
	assert_eq!(
		auth_url
			.query_pairs()
	...
	assert_eq!(
		auth_url
			...
			.map(|(_, v)| v.into_owned())
			.as_deref(),
		Some("https://example.test/_gateway/route-callback")
	);

	let callback = complete_callback_at_path(
		&client,
		&gateway,
		"/_gateway/route-callback",
		"route-code-1",
		&state,
		&nonce_cookie,
	)
...
#[tokio::test]
async fn route_scoped_oauth2_shared_callback_uses_state_to_select_owner() {
	let Some((gateway, oidc, client)) =
		setup_gateway_with_listener_and_route_oauth2_shared_callback(true).await
	else {
		return;
	};
	...
	assert_eq!(
		auth_url
			...
			.map(|(_, v)| v.into_owned())
			.as_deref(),
		Some("https://example.test/_gateway/callback")
	);
...
async fn setup_gateway_with_listener_and_route_oauth2(
	backend_passthrough: bool,
) -> Option<(AgentGateway, MockServer, reqwest::Client)> {
	if !require_e2e() {
		return None;
	...
	let gw = AgentGateway::new(listener_and_route_oauth2_config(
		&oidc.uri(),
		&upstream.address().to_string(),
		backend_passthrough,
	))
...
async fn setup_gateway_with_listener_and_route_oauth2_shared_callback(
	backend_passthrough: bool,
) -> Option<(AgentGateway, MockServer, reqwest::Client)> {
	if !require_e2e() {
		return None;
	...
	let gw = AgentGateway::new(listener_and_route_oauth2_shared_callback_config(
		&oidc.uri(),
		&upstream.address().to_string(),
		backend_passthrough,
	))
...
pub(super) async fn complete_callback_at_path(
	client: &reqwest::Client,
	gateway: &AgentGateway,
	path: &str,
	code: &str,
	...
	client
		.get(callback_url(gateway, path, code, state))
		.header(reqwest::header::COOKIE, nonce_cookie)
		.send()
		.await
		.unwrap()
...
fn callback_url(gateway: &AgentGateway, path: &str, code: &str, state: &str) -> Url {
	let mut callback_url = Url::parse(&gateway_url(gateway, path)).unwrap();
	callback_url
		.query_pairs_mut()
...
fn listener_and_route_oauth2_config(
	issuer: &str,
	upstream: &str,
	backend_passthrough: bool,
) -> String {
    ...
    - name: oauth2
      policies:
        oauth2:
          ...
          redirectUri: "https://example.test/_gateway/route-callback"
          scopes:
          - openid
          - profile
{backend_auth_block}      backends:
...
fn listener_and_route_oauth2_shared_callback_config(
	issuer: &str,
	upstream: &str,
	backend_passthrough: bool,
) -> String {
